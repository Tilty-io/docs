...
2.  **Corriger votre code HTML :**
    Si la suggestion est incohérente (par exemple un Objet au lieu d'un Texte), vérifiez s'il n'y a pas d'ambiguïté dans votre code.
    *   *Exemple :* Un `ty-list="..."` oublié sur un champ texte ? Retirez-le pour que Tilty suggère correctement du texte.
    *   *Exemple :* L'usage de `monChamp.valeur` force la création d'un objet `monChamp`.
...
Le moteur d'analyse utilise une série d'indices pour *deviner* le type de champ. Ce ne sont pas des règles absolues, mais des heuristiques qui fonctionnent dans la plupart des cas courants.
